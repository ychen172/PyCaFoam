...
xRefine = list(np.arange(0,2.2,0.2)) #Vertices along X direction
yRefine = list(np.arange(0,1.1,0.1)) #Vertices along Y direction
...
for i in range(0,numOut):
    ...
    ###Predictor Step
    velVec = np.linalg.solve(ATot , np.matmul(APTot,preVec)+bTot) #v* from here
    ###Relaxation Step
    ...
    for j in range(0,numIn):
        ...
        preVec = np.linalg.solve(AAP, np.matmul(AH,velVec)-AB-BS) #P n+1 from V*
        ### Correct velocity
